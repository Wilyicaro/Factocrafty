...
public class FactocraftyStorageMenu<T extends BlockEntity> extends AbstractContainerMenu {
    ...
    public FactocraftyStorageMenu(MenuType<?> menuType, int containerId, BlockPos pos, Player player) {
        super(menuType, containerId);
        Level level = player.level();
        Inventory inventory = player.getInventory();

        this.player = player;
        this.storage = (IFactoryExpandedStorage) level.getBlockEntity(pos);
        ...
        if (be instanceof FactocraftyMenuBlockEntity pBe)
    ...
    protected void updateChanges() {
        if (player instanceof ServerPlayer sp) {
            ...
            storage.itemSides().ifPresent((i) -> i.forEach((d,s) -> Factocrafty.NETWORK.sendToPlayer(sp,new FactocraftyStateButtonPacket(be.getBlockPos(),0,d.ordinal(),s.transportState, s.getSlotIndex(storage.getSlotsIdentifiers())))));
            storage.energySides().ifPresent((i) -> i.forEach((d,s) -> Factocrafty.NETWORK.sendToPlayer(sp,new FactocraftyStateButtonPacket(be.getBlockPos(),1,d.ordinal(),s,0))));
            storage.fluidSides().ifPresent((i) -> i.forEach((d,s) -> Factocrafty.NETWORK.sendToPlayer(sp,new FactocraftyStateButtonPacket(be.getBlockPos(),2,d.ordinal(),s.transportState,s.fluidHandler.identifier().differential()))));


            if (storage instanceof FactocraftyMenuBlockEntity be) be.syncAdditionalMenuData(this, sp);
        }
    }
@Override
